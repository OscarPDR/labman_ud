...
def analyze(G):
    ...
    for cc in components:
        ...
        for name in cc.nodes():
            G.node[name]['cc-betweenness'] = float(cent_betweenness[name])
            G.node[name]['cc-eigenvector'] = float(cent_eigenvector[name])
            G.node[name]['cc-closeness'] = float(cent_closeness[name])
    ...
    degrees = G.degree()
    for name in degrees:
        G.node[name]['degree'] = float(degrees[name])
    ...
    for name in G.nodes():
        G.node[name]['betweenness'] = float(betweenness[name])
        G.node[name]['eigenvector'] = float(eigenvector[name])
        G.node[name]['closeness'] = float(closeness[name])
        G.node[name]['pagerank'] = float(pagerank[name])

    for pos, k_clique in enumerate(k_cliques):
        for member in k_clique:
            G.node[member]['k-clique'] = float(pos)

    partitions = community.best_partition(G)

    for key in partitions.keys():
        G.node[key]['modularity'] = float(partitions[key])
